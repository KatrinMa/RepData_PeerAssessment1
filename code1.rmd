...
```{r System language, echo=TRUE,results="hide"}
...
```{r steps per day / unimputed, echo=TRUE}
...
``` {r mean+median / unimputed, echo=TRUE}
...
``` {r avgerage steps per interval, echo=TRUE}
...
``` {r interval with maximum no. of steps, echo=TRUE}
...
``` {r check missing data, echo=TRUE}
...
```{r steps per day / imputed, echo=TRUE}
...
``` {r histogram unimputed, echo=TRUE}
hist(steps_per_day$no_of_steps, breaks=10,main = "Histogram of no. of steps per day", xlab="No. of steps per day")
```
...
``` {r def of type_of_day / imputed, echo=TRUE}
...
``` {r comparision of weekend and weekdays / imputed, echo=TRUE}
